...
def load_qa_pairs(limit: int | None = None) -> list[QAPair]:
    ...
    split = dataset["validation"] if "validation" in dataset else next(iter(dataset.values()))
    pairs: list[QAPair] = []
    for i, row in enumerate(split):
        question = str(first_present(row, ["question", "query"], ""))
        answer = normalize_answer_value(first_present(row, ["answer", "answers"], ""))
        evidence = first_present(row, ["evidence", "evidences", "highlighted_evidence"], [])
        pairs.append(QAPair(str(first_present(row, ["question_id", "id"], i)), question, answer, list(evidence) if isinstance(evidence, list) else [str(evidence)]))
